...
class ResCompany(models.Model):
    ...
    fr_vat_bank_account_id = fields.Many2one(
        ...
        check_company=True,
        ondelete="restrict",
        help="Company bank account used to pay VAT or "
        "receive credit VAT reimbursements.",
    )
    ...
    def _fr_vat_remind_auto_generate_and_transmit(
        ...
        if vat_return.state == "manual":
            ...
            vat_return.message_post(
                body=_(
                    "Automatic lines have been automatically generated "
    ...
    def _setup_l10n_fr_coa_vat_company(self):
        ...
        sale_tax_dest_id = self.env.ref(f"account.{self.id}_tva_sale_0_exo").id
    ...
    def _test_create_invoice_with_payment(
        ...
        apro = self.env["account.payment.register"].with_company(self.id)
        vals = {
            "company_id": self.id,
            "move_type": move_type,
            "invoice_date": date,
