    ...
    for company in rco.search([]):
        ...
        if not fr_fps:
            logger.info(
                ...
                "fr_vat_type=france", company.name)
            continue
        new_fp = afpo.create({
        ...
        logger.info('New fiscal position france_vendor_vat_on_payment created in company %s', company.name)
        partners_to_update = env['res.partner'].with_company(company_id).browse(
            partner_ids)
        for partner in partners_to_update:
            if not partner.property_account_position_id or partner.property_account_position_id.id in fr_fps.ids:
                partner.write({'property_account_position_id': new_fp.id})
                logger.info('Partner %s now has the new fiscal position france_vendor_vat_on_payment in company %s', partner.display_name, company.name)
        openupgrade.logged_query(
            env.cr,
            'UPDATE account_move SET fiscal_position_id=%%s '
            'WHERE %(move_old_col)s IS true AND '
            'company_id=%%s AND '
            '(fiscal_position_id IS null OR fiscal_position_id IN %%s)' % {'move_old_col': move_old_col}, args=(new_fp.id, company_id, tuple(fr_fps.ids)))
