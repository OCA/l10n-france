...
class PurchaseOrderLine(models.Model):
    ...
    @api.depends('taxes_id', 'product_id', 'product_qty')
    def _compute_ecotaxe(self):
        for line in self:
            price = line._calc_line_base_price(line)
            qty = line._calc_line_quantity(line)
            ecotaxe_id = [tax.id for tax in line.taxes_id if tax.is_ecotaxe]
...
class PurchaseOrder(models.Model):
    ...
    @api.multi
    @api.depends('amount_untaxed', 'order_line.taxes_id',
    ...
    def _compute_ecotaxe(self):
        for order in self:
            ...
            order.amount_untaxed_with_ecotaxe = \
                order.amount_untaxed + order.amount_ecotaxe
            order.amount_tax_without_ecotaxe = \
                order.amount_tax - order.amount_ecotaxe
