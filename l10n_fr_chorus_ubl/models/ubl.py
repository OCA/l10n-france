...
class BaseUbl(models.AbstractModel):
    ...
    def _ubl_get_tax_scheme_dict_from_tax(self, tax):
        ...
        if self._context.get("fr_chorus") and tax.unece_type_code == "VAT":
            res["type_code"] = "TVA"
        return res

    @api.model
    def _ubl_get_tax_scheme_dict_from_partner(self, commercial_partner):
        res = super()._ubl_get_tax_scheme_dict_from_partner(commercial_partner)
    ...
    def _ubl_get_party_identification(self, commercial_partner):
        res = super()._ubl_get_party_identification(commercial_partner)
        # partner.siret has a value even if partner.nic == False
        if (
            self._context.get("fr_chorus")
            and commercial_partner.siren
            and commercial_partner.nic
        ):
