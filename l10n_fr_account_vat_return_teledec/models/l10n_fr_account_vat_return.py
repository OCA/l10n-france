...
class L10nFrAccountVatReturn(models.Model):
    ...
    def send_ca3_via_teledec(self):
        ...
        if self.sent_datetime:
            raise UserError(
                _(
                    ...
                    datetime=format_datetime(self.env, self.sent_datetime),
                )
            )
        ...
        if res.status_code == 200:
            ...
            self.write({"sent_datetime": fields.Datetime.now()})
            fnametime = teledec_dict["auth"]["timestamp"].replace(":", "-")
            filename = f"teledec_data-{fnametime}.json"
