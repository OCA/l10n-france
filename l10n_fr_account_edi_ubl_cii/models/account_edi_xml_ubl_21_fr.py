...
PDP_CUSTOMIZATION_ID = (
    "urn:cen.eu:en16931:2017"  # Not accepted by SuperPDP due to missing validator
)
...
class AccountEdiXmlUbl21Fr(models.AbstractModel):
    ...
    def _export_invoice_constraints_new(self, invoice, vals):
        ...
        for partner_type in ("supplier", "customer"):
            ...
            if (
                commercial_partner.peppol_eas != "0225"
                or not commercial_partner.peppol_endpoint
            ):
                constraints[f"ubl_21_fr_{partner_type}_pdp_identifier_required"] = (
                    self.env._(
                        "The following partner's PDP identifier is missing: %s",
                        commercial_partner.display_name,
                    )
                )
            id_type, id_value = commercial_partner._l10n_fr_get_base_identifier()
            if not id_type or not id_value:
                constraints[f"ubl_21_fr_{partner_type}_siret_required"] = self.env._(
                    "The following partner's SIREN or SIRET is missing: %s",
                    commercial_partner.display_name,
                )
            if not commercial_partner.vat or commercial_partner.vat == "/":
                constraints[f"ubl_21_fr_{partner_type}_vat_required"] = self.env._(
                    "The following partner's VAT is missing: %s",
                    commercial_partner.display_name,
                )

        if vals["document_type"] == "credit_note" and not (
            invoice.reversed_entry_id.name or invoice.reversed_entry_id.invoice_date
        ):
            constraints[f"ubl_21_fr_{partner_type}_refund_invoice_reference"] = (
                self.env._(
                    "The original journal entry's name or issue date are missing: %s",
                    vals["invoice"].name,
                )
            )
    ...
    def _add_invoice_header_nodes(self, document_node, vals):
        ...
        # M1 : Dépôt d'une facture double (livraison de bien et services qui ne sont
        # pas accessoires l'une de l'autre)
        ...
        if invoice.payment_state in PAID_STATES:
            ...
            not invoice._is_downpayment()
            and invoice.invoice_line_ids._get_downpayment_lines()
        ):
            # After downpayment
            profile_number = "4"
    ...
    def _ubl_add_party_identification_nodes(self, vals):
        res = super()._ubl_add_party_identification_nodes(vals)
    ...
    def _ubl_add_party_legal_entity_nodes(self, vals):
        # EXTENDS account.edi.xml.ubl_bis3
        res = super()._ubl_add_party_legal_entity_nodes(vals)
        partner = vals["party_vals"]["partner"]
        commercial_partner = partner.commercial_partner_id
