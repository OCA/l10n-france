...
class ResPartner(models.Model):
    ...
    def _l10n_fr_get_base_identifier(self):
        ...
        siret = self.siret or (
            ...
            if self.company_registry and siren_siret_re.match(self.company_registry)
            else ""
        )
        siren = siret[:9]
        if len(siret) == 9:
    ...
    def _get_suggested_pdp_identifier(self):
        ...
        # "Everyone" will probably have registered the SIREN on annuaire.
        # (Even if they have a SIRET.)
    ...
    def _build_error_peppol_endpoint(self, eas, endpoint):
        ...
        if not self.env["res.company"]._check_pdp_identifier(endpoint):
            return self.env._(
                ...
                "SIREN, SIREN_SIRET, SIREN_SIRET_CodeRoutage or SIREN_SuffixeAdressage"
            )
