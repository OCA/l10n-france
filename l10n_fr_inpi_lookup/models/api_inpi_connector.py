...
from ..inpi.inpi_models import InpiResponse, RolePourEntreprise
...
class ApiInpi(models.Model):
        ...
        try:
            if call_type == "get":
                response = requests.get(
                    url, timeout=self.company_id.inpi_timeout, **kwargs
                )
            elif call_type == "post":
    ...
    def _get_data(self, url, params=None, headers=None):
        ...
        response, response_headers = self.api_get(
            url=self.company_id.inpi_url + url,
            verify=True,
            params=params,
            headers=headers,
        )
    ...
    def _generate_access_token(self):
        if not self.company_id.inpi_user or not self.company_id.inpi_pass:
            raise UserError(_("Inpi acces not configured on company"))

        try:
            response, headers = self.api_post(
                url=self.company_id.inpi_url + "/sso/login",
                verify=True,
                json={
                    "username": self.company_id.inpi_user,
                    "password": self.company_id.inpi_pass,
                },
            )
